...
`argparse` 모듈을 사용해 명령행 인자로부터  TCP 포트를 취한다. 서버와 클라이언트 스크립트 모두 이 포트 정보가 필요하다.

#### Server

TCP  소켓 객체를 만든다. 그 후 주소 재사용 옵션을 설정하여 필요할 때마다 서버를 재시작한다. 주어진 포트에 소켓을 바인딩한다.
`listen()` 메소드의 백로그 인자를 사용하여 요청 대기 구현 단계에서 큐를 이용해 다수의 클라이언트에 응답한다.
마지막으로 클라이언트가 접속해 데이터를 서버에 전송되길 기다리다 데이터를 수신하면 그 데이터를 다시 클라이언트로 돌려보낸다.

#### Client
포트 번호를 명령행 인자에서 얻어 소켓을 생성한 후 서버에 접속한다.
