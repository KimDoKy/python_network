...
class PortForwarder(asyncoer.dispatcher):

    def __init__(self, ip, port, remoteip, remoteport, backlog=5):
        asyncoredispatcher.__init__(self)
    ...
    def handle_accept(self):
        ...
        Sender(Reciver(conn), self.remoteip, self.remoteport)

class Receiver(asyncore.dispathcher):
...
class Sender(asyncore.dispather):

    def __init__(self, receiver, remoteaddr, remoteprot):
        asyncore.dispather.__init__(self)
        self.receiver=receiver
        seceiver.sender=self
        self.create_socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    def handle_read(self):
        read = self.recv(BUFSIZE)
        self.reciver.to_remote_buffer += read
